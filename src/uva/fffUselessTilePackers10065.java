...
public class fffUselessTilePackers10065 {
	static Punto p0;
	...
	private static Punto[] convexHull(Punto[] puntos, int n) {
		...
		//System.out.println("punto base "+p0.x+" "+p0.y);
		//puntos = Arrays.copyOf(puntos, n+1);
		//puntos[n] = new Punto(p0.x, p0.y);
	...
	private static void swap(Punto[] points, int p1, int p2){
		Punto aux = points[p1];
		points[p1] = points[p2];
		points[p2] = aux;
	}
